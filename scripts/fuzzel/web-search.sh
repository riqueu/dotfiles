...
main() {
  # pass the list to fuzzel
  platform=$( (gen_list) | fuzzel --dmenu --lines=10)

  if [[ -z "$platform" ]]; then
    exit
  fi

  # get search query using fuzzel with a dynamic prompt
  # We use `echo "" |` to force fuzzel into dmenu mode with an empty list
  query=$(echo "" | fuzzel --dmenu --prompt="$platform > ")
